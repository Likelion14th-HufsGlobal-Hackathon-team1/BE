...
### `GET /products/scan?code={productCode}`
...
| 항목 | 상태 |
| --- | --- |
| Discover(스타일링 추천) | 담당 미정, 3순위 — 계약 작성 보류 |
| NFC/QR 재구매(기존 모델 추가등록) 분기 | 미해결, 팀 논의 필요 |
| 소셜 로그인 여부 | `POST /auth`를 자체 로그인 기준으로 임시 작성, 확정되면 갱신 |

---

*작성: BE 팀장 고선민 · 2026-08-12. 계약 변경은 FE 조율 후 이 문서 갱신으로만.*
